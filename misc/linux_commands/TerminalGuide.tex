...
\begin{commandshell}
    @\shellprefix@python @\textit{\PYG{l+s}{<programm.py>}}@
\end{commandshell}
...
Alles weitere ist für die \mintinline{text}{ipython}-REPL ausgelegt.

Mit \textit{\%load} kann man Python-Programme auch direkt in eine REPL laden. Wenn wir z.B. eine Datei \mintinline{text}{load.py} haben, können wir diese mit \textit{\%load} laden:
\begin{codeBlock}[]{minted language=kotlin,title=\codeBlockTitle{load.py}}
    print('Hello')
    a = 2+ 4
\end{codeBlock}
\begin{commandshell}[][minted language=text]
