...
\titlegraphic{
    \vbox to \height {%
        \vfill
        \tikzstyle{rect} = [rectangle, rounded corners, minimum width=2.5cm, minimum height=1cm,text centered, draw, text width=5cm, fill=\IfDarkModeTF{TUDa-9b!30!black}{TUDa-9b!10}]
        \centering
        \begin{tikzpicture}[start chain=going below,node distance=.2cm]
            ...
            \node[rect, on chain] {\textbf{Logische Operatoren}};
        \end{tikzpicture}
        \vfill
    }
}
...
\begin{frame}
    \slidehead
    \begin{itemize}[<+->]
        \item Wie können wir aus einem \pythoninline{int} einen \pythoninline{float} machen?
        \item Python ermöglicht Konvertierung: \pythoninline{float(3)}
        \item Analog dazu: \pythoninline{int(3.14)}
    \end{itemize}
\end{frame}
